...
const Blank = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${server}/vendors`); // replace with your API endpoint
        ...
        const fetchData = async () => {
            try {
                ...
                for (const vendor of response.data) {
                    const detailsResponse = await axios.get(`${server}/landing/${vendor.id}/details`);
                    // console.log(detailsResponse.data.product_count);
                }
            } catch (error) {
                console.error("Error fetching data", error);
            }
                                        ...
                                        <div className="vendor-wrap mb-40  style-2">
                                            ...
                                            <div className="vendor-img-action-wrap">
                                                <div className="vendor-img">
                                                    <Link legacyBehavior href={`vendor/${item.id}`}><a>
                                                        <img className="default-img"
                                                             src={`assets/imgs/vendor/${item.img}`} alt=""/>
                                                    </a></Link>
                                                </div>

                                                <div className="mt-10">
                                                    <span className="font-small total-product">{item.product_count || 0} products</span>
                                                </div>
                                            </div>
                                            <div className="vendor-content-wrap">
                                                <div className="mb-30">
                                                    ...
                                                    <h4 className="mb-5"><Link legacyBehavior href={`vendor/${item.id}`}><a>{item.store_name}</a></Link></h4>
                                                    ...
                                                    <div className="vendor-info d-flex justify-content-between align-items-end mt-30">
                                                        ...
                                                        {/*</ul>*/}
                                                        <Link legacyBehavior href={`vendor/${item.id}`}><a className="btn btn-xs">Visit Store <i className="fi-rs-arrow-small-right"></i></a></Link>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
